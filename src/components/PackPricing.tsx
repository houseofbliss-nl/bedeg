...
export function PackPricing({
  ...
  return (
    <div className={compact ? "mt-2 space-y-1.5" : "space-y-2.5"}>
      ...
      <div className={compact ? "grid grid-cols-2 gap-1" : "grid grid-cols-2 md:grid-cols-4 gap-2"}>
        {packs.map((pack) => (
          <span
            key={pack.size}
            className="inline-flex min-w-0 items-center justify-between gap-1 rounded-md border border-[#DDD6FF] bg-white px-2 py-1.5 text-[10px] font-bold text-[#1F1F1F]"
          >
            <span>Pack {pack.size}</span>
            <span className="text-[#5B3DF5]">{formatPackPrice(pack.price)}</span>
            <span className="text-[9px] text-[#16A34A]">-{pack.savingsPercent}%</span>
          </span>
        ))}
      </div>
    </div>
